...
// ---- Rollback: reset to a specific commit + rebuild/restart -------------------
async function rollbackById(configId, commit, userId, onStep) {
  ...
  if (!commit || !/^[a-fA-F0-9]{4,40}$/.test(commit)) throw new Error('Invalid commit hash');
  ...
  const commitBefore = beforeStatus?.currentCommit || null;

  const logResult = await insert(
    'INSERT INTO deploy_logs (connection_id, branch, commit_before, commit_after, deployed_by, status, note) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [configId, branch, commitBefore, commit, userId || null, 'running', `rollback to ${commit.slice(0, 7)}`]
  );
  const deployLogId = logResult.insertId;

  const steps = [
    { name: 'verify-commit', cmd: `git cat-file -e ${commit}^{commit}`, critical: true },
    { name: 'fetch', cmd: `git fetch --quiet ${shellQuote(fetchUrl)} ${shellQuote(branch)}`, critical: true },
    { name: 'reset', cmd: `git reset --hard ${commit}`, critical: true },
  ];
  if (config.install_cmd) steps.push({ name: 'install', cmd: `if git diff --name-only HEAD@{1} HEAD -- package.json package-lock.json 2>/dev/null | grep -q .; then ${config.install_cmd}; else echo "package.json unchanged - skipped"; fi`, critical: true });
  if (config.build_cmd) steps.push({ name: 'build', cmd: config.build_cmd, critical: true });
  if (config.restart_cmd) steps.push({ name: 'restart', cmd: config.restart_cmd, critical: true });
  steps.push({ name: 'verify', cmd: 'git rev-parse --short=7 HEAD', critical: true });
  ...
  try {
    for (const step of steps) {
      ...
      if (onStep) { try { onStep(entry) } catch {} }
      if (!entry.ok && step.critical) {
        const failMsg = step.name === 'verify-commit'
          ? `Commit ${commit.slice(0, 7)} not found on the server`
          : `Step "${step.name}" failed: ${(res.err || res.out || 'exit ' + res.code).slice(0, 300)}`;
        throw Object.assign(new Error(failMsg), { steps: results, commitAfter: null });
      }
    }
  } finally {
    conn.end();
  ...
  const commitAfter = results.find((r) => r.name === 'verify')?.output || commit;
  const duration_ms = Date.now() - started;

  await update(
    'UPDATE deploy_logs SET status = "deployed", commit_after = ?, log_text = ?, deployed_at = NOW() WHERE id = ?',
    [commitAfter, JSON.stringify(results), deployLogId]
  );
  await update(
    'UPDATE remote_deploy_configs SET last_commit = ?, last_deployed_at = NOW() WHERE id = ?',
    [commitAfter, configId]
  );

  try {
    await notifyDeployExecuted({ name: config.name }, { commit: commitAfter, duration: duration_ms, rollback: true });
  } catch {}

  return { ok: true, commitBefore, commitAfter, steps: results, duration_ms, deployLogId };
}
