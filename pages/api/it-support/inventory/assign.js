  ...
  if (req.method === 'POST') {
    ...
    if (action === 'unassign') {
      ...
      try {
        ...
        await db.query("UPDATE it_inventory SET status = 'available' WHERE id = ?", [inventory_id])
    ...
    try {
      await db.query(
        'UPDATE it_inventory_assign SET returned_at = NOW() WHERE inventory_id = ? AND returned_at IS NULL', [inventory_id])
      await db.insert('it_inventory_assign', {
        inventory_id, user_id, assigned_by: user.id, notes: notes || null,
      })
      await db.query("UPDATE it_inventory SET status = 'in_use' WHERE id = ?", [inventory_id])
