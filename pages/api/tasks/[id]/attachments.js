...
export default async function handler(req, res) {
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      const existing = await db.queryOne('SELECT * FROM task_attachments WHERE id = ? AND task_id = ?', [attachment_id, id]);
      ...
      await db.query('DELETE FROM task_attachments WHERE id = ?', [attachment_id]);
