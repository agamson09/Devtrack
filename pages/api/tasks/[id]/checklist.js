...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const maxOrder = await db.queryOne('SELECT MAX(sort_order) as max_order FROM task_checklists WHERE task_id = ?', [id]);
      ...
      const item = await db.queryOne('SELECT * FROM task_checklists WHERE id = ?', [result.insertId]);
      try {
  ...
  if (req.method === 'PUT') {
    try {
      ...
      const existing = await db.queryOne('SELECT * FROM task_checklists WHERE id = ? AND task_id = ?', [item_id, id]);
      ...
      if (updates.length > 0) {
        params.push(item_id);
        await db.query('UPDATE task_checklists SET ' + updates.join(', ') + ' WHERE id = ?', params);
      }

      const item = await db.queryOne('SELECT * FROM task_checklists WHERE id = ?', [item_id]);
      try {
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      if (comment_id) {
        await db.query('DELETE FROM task_comments WHERE id = ? AND task_id = ?', [comment_id, id]);
        return res.status(200).json({ message: 'Reply deleted' });
      }

      if (!item_id) return res.status(400).json({ error: 'Item ID is required' });

      await db.query('DELETE FROM task_checklists WHERE id = ? AND task_id = ?', [item_id, id]);
