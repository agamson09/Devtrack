...
export default async function handler(req, res) {
  ...
  try {
    const config = await tenantQueryOne(tenantId, 'SELECT * FROM remote_deploy_configs WHERE id = ?', [id])
    ...
    await tenantQuery(tenantId, 'UPDATE remote_deploy_configs SET last_commit = ? WHERE id = ?', [status.currentCommit, config.id]).catch(() => {})
