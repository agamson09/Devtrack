...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const note = await tenantQueryOne(tenantId,
      ...
      const backlinks = await tenantQuery(tenantId, backSql, backParams)
  ...
  if (req.method === 'PUT') {
    ...
    try {
      const existing = await tenantQueryOne(tenantId, 'SELECT * FROM wiki_notes WHERE id = ?', [id])
      ...
      await tenantUpdate(tenantId, `UPDATE wiki_notes SET ${fields.join(', ')} WHERE id = ?`, params)
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await tenantQuery(tenantId, 'DELETE FROM wiki_notes WHERE id = ?', [id])
